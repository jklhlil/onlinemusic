<template>
  <div class="body" ref="body">
    ...
    <div class="top">
      ...
      <van-icon class="right" name="ellipsis" size="20px" color="#f6fef8" @click="comment" />
    </div>
    ...
    <div class="bottom">
      ...
      <div class="icons1">
        ...
        <!-- 订阅 -->
        <van-icon class="unite subscribe" name="https://s1.ax1x.com/2022/06/29/jnbnaR.png" size="26px" @click="subscribe" :badge="musicData.music.score===0?null:musicData.music.score" v-if="musicData.isSubscribe === 1" />
        <van-icon class="unite subscribe" name="https://s1.ax1x.com/2022/06/29/jnbZqJ.png" size="26px" @click="subscribe" :badge="musicData.music.score===0?null:musicData.music.score" v-else />

        <van-icon class="unite download" name="https://s1.ax1x.com/2022/06/29/jnbuI1.png" @click="download" ref="down" />
      ...
      <div class="progress">
        ...
        <van-slider class="lin line" v-model="progressValue" bar-height="1px" active-color="#ffffff" inactive-color="rgba(207, 207, 207, 0.553)" @change="onProgressChange">
          <template #button>
        ...
        <span class="lin totalTime">{{ totalTimeString }}</span>
      </div>
      ...
      <div class="icons2">
        ...
        <van-icon class="unite loop" name="https://s1.ax1x.com/2022/06/29/jnH75t.png" @click="loop(1)" v-if="isSingle===0" />
        <van-icon class="unite loop" name="https://s1.ax1x.com/2022/06/29/jnHL28.png" @click="loop(0)" v-else />
...
export default {
    ...
    return {
      ...
      musicData: {
        ...
        music: {
          name: "画",
          singer: "邓紫棋",
          url: "http://localhost:8089/files/source/%E9%82%93%E7%B4%AB%E6%A3%8B%20-%20%E7%94%BB.mp3",
        },
      },
      user: {},
      commentNum: 0,
  ...
  created() {
    this.getUser();
    if (Number(this.$route.query.state) === 1) {
      // 从列表点进来，重头播放
      // sessionStorage.setItem("isAllow", 0);
      this.init();
    } else {
      // 其他方式进来，从currentTime播放
      // sessionStorage.setItem("isAllow", 1);
      this.init2();
    }
  },
  destroyed() {
    ...
    // 2.音乐的信息
    sessionStorage.setItem("musicUrl", this.musicData.music.url);
    sessionStorage.setItem("currentTime", this.audioMusic.currentTime);

    sessionStorage.setItem("musicIndex", this.index);
  ...
  methods: {
    ...
    init2() {
      ...
      this.index = Number(sessionStorage.getItem("musicIndex"));
    ...
    download() {
      //未订阅，无法下载
      if (this.musicData.isSubscribe != 1 && this.musicData.music.score > 0) {
        Toast("无法下载，请先订阅。");
        return 0;
      }
    ...
    executeIntervals() {
      ...
      id = setInterval(() => {
        ...
        if (
          this.musicData.isSubscribe != 1 &&
          this.musicData.music.score > 0 &&
          this.audioMusic.currentTime >= 60
        ) {
          Toast("60S试听结束，请先订阅。");
          this.play();
          this.loopSet();
    ...
    updateProgress() {
      ...
      this.progressValue = proportion * 100;
      // this.$refs.line1.style.width = `${proportion * 70}%`;
      // this.$refs.line2.style.width = `${(1 - proportion) * 70}%`;
    },
    // 自己改变进度条
    onProgressChange() {
      this.audioMusic.currentTime =
        (this.progressValue * this.audioMusic.duration) / 100;
    },
    // 背景颜色
    getBackground() {
      let bgList = [
...
#status_pic {
  ...
  box-shadow: 0px 0px 1px 5px rgba(255, 255, 255, 0.2333);
